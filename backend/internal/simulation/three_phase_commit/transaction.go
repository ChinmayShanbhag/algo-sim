...
const (
	...
	TxStateCanCommit    TransactionState = "can_commit"    // Phase 1: Asking if can commit
	TxStatePreCommitting TransactionState = "pre_committing" // Phase 2: Pre-committing
	TxStateCommitting   TransactionState = "committing"    // Phase 3: Committing
...
// Transaction represents a distributed transaction in 3PC
...
// CanCommit checks if the transaction can proceed to pre-commit
// In 3PC, ALL participants must vote YES for commit to proceed
...
// PreCommit marks the transaction as pre-committing
func (t *Transaction) PreCommit() {
	t.State = TxStatePreCommitting
}
...
func (t *Transaction) GetResult() string {
	...
	case TxStatePreCommitting:
		return fmt.Sprintf("Transaction %s in pre-commit phase", t.ID)
	case TxStateCanCommit:
		return fmt.Sprintf("Transaction %s in can-commit phase (%d/%d votes received)", t.ID, t.YesVotes+t.NoVotes, t.TotalVotes)
	default:
